...
clone()、getClass()、toString()、finalize()、euqals()、hashCode()、wait()、notify()、notify()、notifyAll()、
...
+ 某个类只能有一个实例（构造器私有化）
+ 它必须自行创建这个实例（用静态变量保存这个唯一的实例）
+ 必须自行向整个系统提供这个实例（①直接暴露或者②用静态变量的get方法获取）
...
+ 懒汉式：延迟创建对象
  ...
  线程不安全（适用于单线程），用静态方法获取这个示例对象；
  线程安全（适用于多线程），静态方法内部加synchronized；
  静态内部类形式（适用多线程）。
